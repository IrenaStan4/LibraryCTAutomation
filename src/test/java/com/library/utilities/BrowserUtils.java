...
public class BrowserUtils {
    ...
    Argument 2 => expectedURL: to verify if the URL contains given String
                  If condition matches, loop will break;
    Argument 3 => expectedTitle to be compared against actualTitle
     */
    public static void switchWindowAndVerify(String expectedURL, String expectedTitle){
        Set<String> allWindowHandles =Driver.getDriver().getWindowHandles();

        for (String each : allWindowHandles) {  //iterating through all windows
            Driver.getDriver().switchTo().window(each);    //driver will switch to every window
            System.out.println("Current URL " + Driver.getDriver().getCurrentUrl());    //get current URL
    ...
    //This method accepts 1 String expectedTitle and asserts it is true
     ...
     * and return the options text in a list of String
    ...
    public static void highlight(WebElement element) {
        ((JavascriptExecutor) Driver.getDriver()).executeScript("arguments[0].setAttribute('style', 'background: #FDFDCB; border: 2px solid green;');", element);
        waitFor(1);
        ((JavascriptExecutor) Driver.getDriver()).executeScript("arguments[0].removeAttribute('style', 'background: #FDFDCB; border: 2px solid green;');", element);
    }
